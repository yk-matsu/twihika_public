...
export const handleSaveRegisteredUsersTwitterQuery = async (
  client: Client,
  s3: AWS.S3,
) => {
  const notify: string[] = [];
  ...
  try {
    ...
    for (const masterQuery of sinceIdAndQueries) {
      ...
      await topic.publishMessage({
        json: {
          pattern: 'nestjs',
          type: 'REGISTERED_batch_twitter_user_search_queries',
      ...
      await client.query(
        `
        delete from batch_twitter_search_users_divided_into_30 where count < $1 and twitter_user_id = $2
      `,
        [threshold, masterQuery.twitter_user_id],
      );
      ...
      console.log(await client.query(
        'insert into batch_search_users_processing_state (twitter_user_id) values($1)',
        [masterQuery.twitter_user_id],
      ));
    }
    await client.query('DELETE FROM batch_search_users_processing_state')
    await client.end();
  } catch (error) {
    console.log(error)
    throw error;
