...
export const validateTweetMetaFromPostgres = (data) => {
  ...
  const constraints: JSONSchemaType<TweetMetaFromPostgres[]> = {
    ...
    items: {
      ...
      required: ['s3_bucket', 's3_key', 'twitter_user_id'],
    },
  };
...
const fetchChunckedTweets3Location = async (client: Client) => {
  ...
  SELECT
    ...
    m.twitter_user_id
  FROM
    master_batch_search_users AS m
    INNER JOIN batch_twitter_search_users_divided_into_30 AS d ON m.twitter_user_id = d.twitter_user_id
  WHERE
...
export const handleSaveRegisteredUserTweetFromS3ToElastic = async (
  client: Client,
  s3: AWS.S3,
    ...
    for (const masterQuery of tweetsS3LocationMetas) {
      ...
      try {
        for (const tweet of tweets) {
          ...
          // const extendedTweet = addQueryIdToUser(tweet, [masterQuery.twitter_user_id], masterQuery.query_category_id ? [masterQuery.query_category_id] : []);
          const res = await axiosElsaticClient.put(
            `/${TWEET_USERS_INDEX}/_doc/${tweet.user.id_str}`,
            tweet.user,
          );
          if (res.status == 201) {
            const pubsub = new PubSub({});
      ...
      await client.query(
        format(
          `
       UPDATE batch_twitter_search_users_divided_into_30 SET is_uploaded_to_elastic = TRUE
       WHERE s3_bucket = %L
       AND s3_key IN (%L)
      `,
